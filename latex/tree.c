...
/*\fn return_hight(node_t*node)
 *\param  node pointer to  node_t
 * The function return node height
 */
int return_hight(node_t*node){
	if (node == NULL)
		return 0;
	return node -> height; 
}
...
node_t**insertaux(node_t**node, void *key){ 
	...
	if(compare((*node) ->element, key) == 0){
		return node; 
	}
	else if(compare((*node)->element, key) <0){

		insertaux(&((*node)->left), key);
	}
	
	else{
 
		insertaux(&((*node) ->right), key); 
	}
	return node; 
}
...
node_t**insertNode(tree_t *tree, void*key){
	if(tree -> root == NULL){
		tree -> root =  node_new();
		tree -> root -> element = key; 
		tree -> root -> left = NULL;
		tree -> root -> right = NULL;
		return &(tree->root); 	
	}
	insertaux(&(tree -> root), key); 
}
...
node_t*findaux(node_t *node, void*Itemname){
	if (node == NULL)
		...
		if(compare(get_name(node->element), Itemname)== 0){
			return node;
		}
		else if(compare(get_name(node-> element), Itemname) > 0){
...
*/
void dumpTree(node_t *node, char *array[], int *p_t){
    if(node){
      dumpTree(node-> left, array, p_t);
      array[*p_t] = get_name(returnNodeware(node));
     * p_t += 1; 
...
void sortNode(char*nodeArray[], int size, node_t*node){
	int i, j= 0;
	dumpTree(node, nodeArray, &j); 
	qsort(nodeArray, size -1, sizeof(char*), compareString); 	
}
...
node_t**balance_node(node_t**node, char*array[], int first, int last){
	int middel = (first + last)/2;
	
    if (first < last){
		*node = node_new(); 
		(*node)-> element = array[middel];
		(*node)-> left = NULL;
		(*node)-> right = NULL;
		insertaux(node, array[middel]); 
		balance_node(&((*node)->left),array, first, middel -1);
		balance_node(&((*node)->  right), array, middel +1, last);
	}
	else {
		return NULL; 
	}
...
void blanced_tree(tree_t*tree){
	...
	char*array[size];
	sortNode(array, size, tree->root);
	balance_node(&(t ->root) , array, 0, size); 
